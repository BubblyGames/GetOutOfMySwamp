        ...
        for (int i = 0; i < size; i++)
        {
            for (int j = 0; j < size; j++)
            {
                for (int k = 0; k < size; k++)
                {
                    ...
                    if (/*(horizontalNoise + verticalNoise) / 2*/ perlin > (1 - (wallDensity * alpha))) //i == 0 || j == 0 || i == size - 1 || j == size - 1 ||//|| (i == j && i < size - 1)
                    {
        ...
        for (int i = -1; i <= 1; i++)
        {
            for (int j = -1; j <= 1; j++)
            {
                for (int k = -1; k <= 1; k++)
                {
                    ...
                    if (isPosInBounds(x,y,z) && (cells[x, y, z].blockType == BlockType.Air || cells[x, y, z].blockType == BlockType.Swamp))
                    {
